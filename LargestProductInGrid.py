...
print matrix
print "finding maximum product of four linearly adjacent numbers"
# Make it take the sections I want. I need horizontal, vertical, and diagonally
# left and right
...
for rows in range(16):
    for columns in range(16):
        listVertical.append(vertical(rows,columns))

for rows in range(16):
    for columns in range(16):
        listDiagRt.append(diagRight(rows,columns))
# starting on the left and going diagonally left with the index starting at (0,0)
# won't return a tuple, have to start that one at (0,4)
for rows in range(16):
    for columns in range(3,20):
        listDiagLeft.append(diagLeft(rows,columns))

print "Horizontal max is: " + str(max(listHorizontal))
